...
class JOSIEv4o(nn.Module):
    def __init__(self, **args):
        ...
        self.resoner_path = os.path.join(self.args['pretrained_ckpt_path'], 'vicuna_ckpt', self.args['vicuna_version'])
        print(f'Initializing language decoder from {self.resoner_path} ...')

        self.resoner = LlamaForCausalLM.from_pretrained(self.resoner_path)
        ...
        self.resoner_tokenizer = LlamaTokenizer.from_pretrained(self.resoner_path, use_fast=False)
        ...
        self.embeds_to_modality = nn.ModuleList([])
        for layer_idx in self.args['text_emb_to_img_layers']:
            if layer_idx == -1 or layer_idx == self.resoner.config.num_hidden_layers:
                in_dim = self.resoner.config.hidden_size
                self.embeds_to_modality.append(DecoderProjector(in_dim, 768, num_input_tokens=self.args['num_gen_img_tokens'], num_output_tokens=self.args['num_clip_tokens'], mode=self.args['text_fc_to_img_mode']))
            # self.sd_pipe.text_encoder.config.hidden_size
            elif layer_idx < self.resoner.config.num_hidden_layers:
                self.embeds_to_modality.append(DecoderProjector(self.resoner.config.hidden_size, 768, num_input_tokens=self.args['num_gen_img_tokens'], num_output_tokens=self.args['num_clip_tokens'], mode=self.args['text_fc_to_img_mode']))
            else:
                raise ValueError(
                    f'Embedding of layer {layer_idx} was requested but model only has {self.resoner.config.num_hidden_layers} layers.')

        # the alignment module for LLM-TO-VIDEO
        self.vd_ckpt_path = self.args['video_diffusion']
        self.embeds_to_modality_video = nn.ModuleList([])
        for layer_idx in self.args['text_emb_to_video_layers']:
            if layer_idx == -1 or layer_idx == self.resoner.config.num_hidden_layers:
                in_dim = self.resoner.config.hidden_size  # 4096
                self.embeds_to_modality_video.append(DecoderProjector(in_dim, 1024, num_input_tokens=self.args['num_gen_video_tokens'], num_output_tokens=self.args['num_clip_tokens'], mode=self.args['text_fc_to_video_mode']))
            # self.vd_pipe.text_encoder.config.hidden_size
            elif layer_idx < self.resoner.config.num_hidden_layers:
                self.embeds_to_modality_video.append(DecoderProjector(self.resoner.config.hidden_size, 1024, num_input_tokens=self.args['num_gen_video_tokens'], num_output_tokens=self.args['num_clip_tokens'], mode=self.args['text_fc_to_video_mode']))
            else:
                raise ValueError(
                    f'Embedding of layer {layer_idx} was requested but model only has {self.resoner.config.num_hidden_layers} layers.')
        ...
        self.embeds_to_modality_audio = nn.ModuleList([])
        for layer_idx in self.args['text_emb_to_audio_layers']:
            if layer_idx == -1 or layer_idx == self.resoner.config.num_hidden_layers:
                in_dim = self.resoner.config.hidden_size
                self.embeds_to_modality_audio.append(DecoderProjector(in_dim, 512, num_input_tokens=self.args['num_gen_audio_tokens'], num_output_tokens=1, mode=self.args['text_fc_to_audio_mode']))
            # self.ad_pipe.text_encoder.config.projection_dim
            elif layer_idx < self.resoner.config.num_hidden_layers:
                self.embeds_to_modality_audio.append(DecoderProjector(self.resoner.config.hidden_size, 512, num_input_tokens=self.args['num_gen_audio_tokens'], num_output_tokens=1, mode=self.args['text_fc_to_audio_mode']))
            else:
                raise ValueError(
                    f'Embedding of layer {layer_idx} was requested but model only has {self.resoner.config.num_hidden_layers} layers.')

        if self.args.get('freeze_output_proj'):
            for name, param in self.embeds_to_modality.named_parameters():
                param.requires_grad = False
            for name, param in self.embeds_to_modality_video.named_parameters():
                param.requires_grad = False
            for name, param in self.embeds_to_modality_audio.named_parameters():
                param.requires_grad = False

    def _add_image_token(self):
        # Add an image token for loss masking (and visualization) purposes.
        self.resoner_tokenizer.add_tokens(["<|image_start|>"])  # add special image token to tokenizer
        self.resoner_tokenizer.add_tokens(["<|image_end|>"])  # add special image token to tokenizer

        # Add <|image_start|> tokens to the vocabulary.
    ...
    def prompt_wrap(self, img_embeds, input_ids, target_ids, attention_mask):
        ...
        bos = torch.ones([batch_size, 1], dtype=input_ids.dtype,
                         device=input_ids.device) * self.resoner_tokenizer.bos_token_id  # bsz x 1
        if self.args['freeze_lm']:
            p_after_embeds = self.resoner.model.embed_tokens(input_ids).expand(batch_size, -1,
                                                                                   -1)  # bsz x s2 x embed_dim
            bos_embeds = self.resoner.model.embed_tokens(bos)  # bsz x 1 x embed_dim
        else:
            p_after_embeds = self.resoner.model.model.embed_tokens(input_ids).expand(batch_size, -1,
                                                                                         -1)  # bsz x s2 x embed_dim
            bos_embeds = self.resoner.model.model.embed_tokens(bos)  # bsz x 1 x embed_dim
        if img_embeds is not None:
            p_before = '### Human: <Img>'
            p_before_tokens = self.resoner_tokenizer(p_before, return_tensors="pt", add_special_tokens=False).to(
                self.device)
            # peft model need deeper call
            if self.args['freeze_lm']:
                p_before_embeds = self.resoner.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1,
                                                                                                        -1)  # bsz x s1 x embed_dim
            else:
                p_before_embeds = self.resoner.model.model.embed_tokens(p_before_tokens.input_ids).expand(
    ...
    def _train_with_mode(self, texts, img_embeds=None, modality='text', num_gen_tokens='8',
        ...
        """
        if stage == 2:
            input_ids, target_ids, attention_mask = process_batch_stage_2(self.resoner_tokenizer, texts,
                                                                          self.max_length,
                                                                          num_gen_tokens,
                                                                          modality
                                                                          )
        elif stage == 3:
            input_ids, target_ids, attention_mask = process_batch_stage_3(self.resoner_tokenizer, texts, self.max_length,
    ...
    def _enc_align_training_stage_1(self, inputs):
        ...
        if dataset_type == 'ImageToText':
            ...
            raise NotImplementedError
        input_ids, target_ids, attention_mask = process_batch_stage_1(self.resoner_tokenizer,
                                                                      inputs['output_texts'],
                                                                      self.max_length,
                                                                      self.args['prompt'])
        # print(input_ids)
    ...
    def _dec_align_training_stage_2(self, inputs):
        ...
        if dataset_type == 'TextToImage':
            ...
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            ...
                                                            text_hidden_fcs=self.embeds_to_modality_video,
            ...
            loss, gen_acc, mse_loss = self._train_with_mode(texts=inputs['output_texts'],
                                                            ...
                                                            text_hidden_fcs=self.embeds_to_modality_audio,
    ...
    def _prepare_image_embed(self, text, batch_size):
        ...
        features = []
        p_before_token = self.resoner_tokenizer('<Img>', add_special_tokens=False, return_tensors='pt').to(self.device)
        p_after_token = self.resoner_tokenizer('</Img>', add_special_tokens=False, return_tensors='pt').to(self.device)
        if self.args['freeze_lm']:
            p_before_embeds = self.resoner.model.embed_tokens(p_before_token.input_ids).expand(batch_size, -1,
                                                                                                   -1)  # bsz x s1 x embed_dim
            p_after_embeds = self.resoner.model.embed_tokens(p_after_token.input_ids).expand(batch_size, -1,
                                                                                                 -1)  # bsz x s2 x embed_dim
        else:
            p_before_embeds = self.resoner.model.model.embed_tokens(p_before_token.input_ids).expand(batch_size, -1,
                                                                                                         -1)  # bsz x s1 x embed_dim
            p_after_embeds = self.resoner.model.model.embed_tokens(p_after_token.input_ids).expand(batch_size, -1,
                                                                                                       -1)  # bsz x s2 x embed_dim
        for m in matches:
            print('image path: ', m)
        ...
        for st in split_text:
            if st.startswith('Image>'):
                ...
                bos = torch.ones([batch_size, 1],
                                 ...
                                 device=text_tokens.input_ids.device) * self.resoner_tokenizer.bos_token_id  # bsz x 1
                if self.args['freeze_lm']:
                    text_embeds = self.resoner.model.embed_tokens(text_tokens.input_ids).expand(batch_size, -1, -1)
                    bos_embeds = self.resoner.model.embed_tokens(bos)  # bsz x 1 x embed_dim
                else:
                    text_embeds = self.resoner.model.model.embed_tokens(text_tokens.input_ids).expand(batch_size,
                                                                                                          -1, -1)
                    bos_embeds = self.resoner.model.model.embed_tokens(bos)  # bsz x 1 x embed_dim
    ...
    def generate_images(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        all_gen_idx: the index of <|image_0|> in the generated_ids
        """
        last_ret_idx = 0
        return_outputs = []
        ...
        for gen_idx in all_gen_idx:
            ...
            gen_prefix = ' '.join([f'<|image_{i}|>' for i in range(self.args['num_gen_img_tokens'])])
            gen_prefx_ids = self.resoner_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
            gen_emb = self.embeds_to_modality[-1](raw_emb, gen_prefix_embs)  # (1, 77, 768)
            ...
            caption = \
                self.resoner_tokenizer.batch_decode(generated_ids[:, last_ret_idx:gen_idx], skip_special_tokens=True)[
                    0]
    ...
    def generate_videos(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        all_gen_idx: the index of <|video_{i}|> in the generated_ids
        """
        return_outputs = []
        last_ret_idx = 0
        ...
        for gen_idx in all_gen_idx:
            ...
            # Produce generation embedding.
            gen_prefix = ' '.join([f'<|video_{i}|>' for i in range(self.args['num_gen_video_tokens'])])
            gen_prefx_ids = self.resoner_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
            gen_emb = self.embeds_to_modality_video[-1](raw_emb, gen_prefix_embs)  # (1, 77, 768)
    ...
    def generate_audios(self, generated_ids, embeddings, all_gen_idx, generation_model=None,
        ...
        for gen_idx in all_gen_idx:
            ...
            # Produce generation embedding.
            gen_prefix = ' '.join([f'<|audio_{i}|>' for i in range(self.args['num_gen_audio_tokens'])])
            gen_prefx_ids = self.resoner_tokenizer(gen_prefix, add_special_tokens=False,
                                                 return_tensors="pt").input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(gen_prefx_ids)  # (1, T_I_V_A.txt, D)
            gen_emb = self.embeds_to_modality_audio[-1](raw_emb, gen_prefix_embs)  # (1, 77, 768)
    ...
    def generate(self, inputs):
        ...
        # Find up to max_num_rets <|image_start|> tokens, and their corresponding scores.
        ...
        # Find up to max_num_rest <|video_start|> tokens, and their corresponding scores.
        ...
        # Find up to max_num_rest <|audio_start|> tokens, and their corresponding scores.
        ...
        if len(all_gen_img_idx) == 0 and len(all_gen_vid_idx) == 0 and len(all_gen_aud_idx) == 0:
            # No <|image_start|>, <|video_start|>, <|audio_start|> tokens.
